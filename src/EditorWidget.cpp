...
void EditorWidget::setProjectPath(const QString& projectPath)
{
        ...
        foreach (const QString& file, srcDir.entryList(QDir::NoDot | QDir::NoDotDot | QDir::Files))
                m_lookup << "src/" + file;

        qDebug() << " checking for include files at " << includeDir.absolutePath();
        foreach (const QString& file, includeDir.entryList(QDir::NoDot | QDir::NoDotDot | QDir::Files))
                m_lookup << "include/" + file;

        qDebug() << " checking for data files at " << dataDir.absolutePath();
        foreach (const QString& file, dataDir.entryList(QDir::NoDot | QDir::NoDotDot | QDir::Files))
                m_lookup << "data/" + file;
